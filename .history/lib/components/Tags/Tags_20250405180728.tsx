...
const Tag = React.forwardRef<HTMLDivElement, TagProps>(
  ({ 
    ...
    const baseClassNames = getTagClassNames(variant, style, size, color);
  ...
  ({
    ...
    return (
      <div
        ...
        {...props}
      >
        <div className={getSplitTagClassNames(style, size, color, true)}>
          {LeftIcon && <LeftIcon className="h-3 w-3" />}
          {leftLabel && <span>{leftLabel}</span>}
        </div>
        <div className={getSplitTagClassNames(style, size, color, false)}>
